...
TIMEOUT_SECONDS="${TRENDINARY_GOOSE_TIMEOUT_SECONDS:-300}"
NAME="trendinary-goose-${MODE}-${GITHUB_RUN_ID:-local}-${GITHUB_RUN_ATTEMPT:-1}-$$"
MACHINE_ID=""
LOG_FILE="$(mktemp -t trendinary-goose-${MODE}.XXXXXX)"
LOG_PID=""
...
if [ -z "$MACHINE_ID" ]; then
  echo "Goose Fly Machine was not created (fly machine run exit=$RUN_STATUS)" >&2
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
  ...
  if [ -n "$EXIT_CODE" ]; then
    ...
    if [ "$EXIT_CODE" -eq 0 ]; then
      if [ "$MODE" = "plan" ]; then
        echo "Goose plan output from Fly Machine ${MACHINE_ID}:"
        print_captured_logs
      fi
      echo "Goose ${MODE} completed successfully in Fly Machine ${MACHINE_ID}."
      exit 0
    fi
    echo "Goose ${MODE} failed in Fly Machine ${MACHINE_ID} with exit code ${EXIT_CODE}." >&2
...
echo "Timed out after ${TIMEOUT_SECONDS}s waiting for Goose ${MODE} Machine ${MACHINE_ID}." >&2
print_captured_logs >&2
exit 124
