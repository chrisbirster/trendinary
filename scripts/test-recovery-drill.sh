...
# Prepare the source exactly as production is prepared: Atlas first, runtime
# writes only after the desired schema exists. The integration test leaves three
# durable signal sentinel rows that must survive the restore.
...
# Stop sqld cleanly, then copy its persistent volume into a fresh volume. This
# models restoring a database snapshot without reusing the original server.
...
# A restore may be from an older point in time. Reconcile it forward with the
# current desired Atlas schema, then prove the source data survived.
...
# Boot the same production image shape against the restored database and run
# the deployment-critical production smoke contract in recovery mode. The
# synthetic fixture has no public trend leaderboard, so only that data-quality
# assertion is skipped; liveness, readiness, runtime state, push, and exact
# release/commit identity remain mandatory.
docker build \
