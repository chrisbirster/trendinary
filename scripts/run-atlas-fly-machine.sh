...
machine_logs() {
  flyctl logs --app "$APP" --machine "$MACHINE_ID" --no-tail 2>/dev/null || true
}
...
while [ "$(date +%s)" -lt "$DEADLINE" ]; do
  ...
  if [ -n "$EXIT_CODE" ]; then
    ...
    if [ "$EXIT_CODE" -eq 0 ]; then
      if [ "$MODE" = "plan" ] && [ -n "$LOGS" ]; then
        printf '%s\n' "$LOGS"
      fi
      echo "Atlas ${MODE} completed successfully in Fly Machine ${MACHINE_ID}." >&2
      exit 0
    fi
