...
// schemaManagedConnector is the production runtime boundary. Goose migrations
// own schema changes, so application connections never send CREATE/ALTER/DROP
// statements to Turso. Legacy package constructors still contain idempotent DDL;
// returning success for schema-only scripts keeps them compatible while making
// the actual production connection schema-read-only.
...
func (c *schemaManagedConn) ExecContext(ctx context.Context, query string, args []driver.NamedValue) (driver.Result, error) {
	...
	if len(statements) > 0 {
		...
		if allDDL {
			if len(args) != 0 {
				return nil, fmt.Errorf("schema mutation is managed by Goose migrations and cannot bind runtime arguments")
			}
