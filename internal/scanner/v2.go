...
func (s *Scanner) RunWithSourcesV2(ctx context.Context, live *recent.Store, extra []DiscoverySource) (Result,error) {
	...
	now:=s.now().UTC();calibration,calibrationErr:=s.history.Calibration(ctx,now.Add(-30*24*time.Hour));if calibrationErr!=nil{warnings=append(warnings,fmt.Sprintf("score calibration: %v",calibrationErr))}
	trends:=make([]model.Trend,0,len(enriched));for _,cluster:=range enriched{entity,err:=s.history.ResolveEntity(ctx,clusterName(cluster),cluster.Key,engine.CanonicalTerms(cluster.Signals,8),now);if err!=nil{warnings=append(warnings,fmt.Sprintf("identity %s: %v",cluster.Key,err));continue};stable:=cluster;stable.Key=entity.ID;trend,snapshot,err:=s.scoreClusterV2(ctx,stable,now,calibration);if err!=nil{warnings=append(warnings,fmt.Sprintf("score %s: %v",entity.ID,err));continue};trend.ID=entity.ID;trend.Slug=entity.Slug;trend.Aliases=entity.Aliases;trend.Name=clusterName(cluster);if err:=s.decorateTrend(ctx,&trend,entity,cluster,now);err!=nil{warnings=append(warnings,fmt.Sprintf("decorate %s: %v",entity.ID,err))};if err:=s.history.RecordSnapshot(ctx,snapshot);err!=nil{warnings=append(warnings,fmt.Sprintf("snapshot %s: %v",entity.ID,err));continue};trends=append(trends,trend)}
