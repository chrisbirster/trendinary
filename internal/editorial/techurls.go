...
func (t *TechURLs) Fetch(ctx context.Context) (FetchResult, error) {
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		return FetchResult{}, fmt.Errorf("techurls returned HTTP %d", resp.StatusCode)
	}
...
func ParseTechURLs(reader io.Reader) (FetchResult, error) {
	...
	if err != nil {
		return FetchResult{}, fmt.Errorf("parse techurls html: %w", err)
	}
	result := FetchResult{}
	...
	walk = func(node *html.Node) {
		if node.Type == html.ElementNode && isSectionNode(node) {
			if label := sectionLabel(node); label != "" {
				currentSection = label
				sections[label] = struct{}{}
			}
		}
		if node.Type == html.ElementNode && node.Data == "a" {
			...
			if title != "" && href != "" && age != "" {
				...
				if !ok {
					...
					if publisher == "" {
						publisher = PublisherDomain(absolute)
					}
...
// ageBefore deliberately stays inside the anchor's own story/list row. Earlier
// versions climbed ancestor siblings and could assign the previous story's age
// to a malformed row, which made stale or incomplete discoveries look valid.
func ageBefore(anchor *html.Node) string {
	if anchor == nil {
		return ""
	}

	// Most TechURLs layouts place the relative age immediately before the link
	// in the same row. Prefer the nearest direct sibling.
	for prev, scanned := anchor.PrevSibling, 0; prev != nil && scanned < 4; prev, scanned = prev.PrevSibling, scanned+1 {
...
func outboundURL(raw string) (string, bool) {
	...
	if host == "techurls.com" || strings.HasSuffix(host, ".techurls.com") || host == "browserling.com" || strings.HasSuffix(host, ".browserling.com") {
		return "", false
	}
