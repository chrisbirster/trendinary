...
func (s *Source) Discover(ctx context.Context) ([]model.Signal, error) {
	...
	// Persist every harvested article, not only clusters that later become a
	// public trend. This makes the free daily allowance useful for replay and
	// corroboration even when a particular story never crosses the trend gate.
